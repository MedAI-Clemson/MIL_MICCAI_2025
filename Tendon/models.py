...
class AchillesNet(nn.Module):
    def __init__(self, encoder, pretrained, pooling_method='attn', num_heads=2, num_features=512, num_out=1, drop_rate=0, debug=False):
        super(AchillesNet, self).__init__()
        self.encoder = encoder
        if pretrained:
            ckpt = torch.load(pretrained)['model_state_dict']
            for layer in ckpt: print(layer)
            self.encoder.load_state_dict(torch.load(pretrained)['model_state_dict'])
            #self.encoder.load_state_dict(torch.load('../Achilles_Classification/models/original_model.ckpt')['model_state_dict'])
        ...
        if pretrained:
            ...
            with torch.no_grad():
                self.fc[1].weight.copy_(checkpoint['model_state_dict']['fc.0.weight'])
                self.fc[1].bias.copy_(checkpoint['model_state_dict']['fc.0.bias'])
        self.debug = debug
