...
from datasets import Xray, MILXray
...
def train_one_epoch_mil(model, optimizer, scheduler, train_loader, criterion, device):
    ...
    for i, batch in enumerate(train_loader):
        inputs = batch['study'].to(device).type(torch.float32)
    ...
    metrics = compute_metrics_multi(np.concatenate(target_ls), np.concatenate(output_ls))
...
def main(cfg):
    ...
    for i, (train_index, val_index) in enumerate(skf.split(targets_df, targets_df['fracture_type'])):
        ...
        # Datasets
        train_df = targets_df.iloc[train_index].merge(imgs_df)
        val_df = targets_df.iloc[val_index].merge(imgs_df)
        view_filter = cfg['view_filter'] #['AP_LIKE', 'Y']
        if cfg['mil']:
            train_dataset = MILXray(train_df, data_path=cfg['data_dir'], transforms=transforms.transform_augment, view_filter=view_filter)
            val_dataset = MILXray(val_df, data_path=cfg['data_dir'], transforms=transforms.transform_original, view_filter=view_filter)
        else:
            train_dataset = Xray(train_df, data_path=cfg['data_dir'], transforms=transforms.transform_augment, view_filter=view_filter)
            val_dataset = Xray(val_df, data_path=cfg['data_dir'], transforms=transforms.transform_original, view_filter=view_filter)
        train_df.to_csv(cfg['save_dir']+f'/train_fold{i+1}.csv')
        val_df.to_csv(cfg['save_dir']+f'/val_fold{i+1}.csv')
