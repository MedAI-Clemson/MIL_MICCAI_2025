...
class ModifiedModel(nn.Module):
    def __init__(self, attention_branches, pretrained_model_path=None, num_classes=3, in_chans=1, dropout=0.3):
        ...
        self.M = 2048  # Feature dimension from ResNet
        self.ff_dim = self.M // 4
        # Load and freeze ResNet backbone
        ...
        # Extract and freeze feature layers
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.M,               # input feature dimension
            nhead=attention_branches,      # number of attention heads
            dim_feedforward=self.ff_dim,        # feedforward network dimension
            dropout=dropout,
        ...
        self.transformer = nn.TransformerEncoder(
            encoder_layer,
            num_layers=2,            
            norm=nn.LayerNorm(self.M)     # final normalization layer
        )
        
        # Final classification layer
        self.classifier = nn.Linear(self.M, num_classes)
        
    def forward(self, x, markers):
        # Extract features from all images
        features = self.features(x).squeeze()  # [total_images, 2048]
        
        outputs = []
        attention_outputs = []  # This will stay empty as we can't access attention weights
        
        # Process each bag separately
        for i in range(len(markers) - 1):
            # Get features for current bag
            bag_features = features[markers[i]:markers[i + 1]]  # [n_i, 2048]
            
            # Add batch dimension for transformer
            bag_features = bag_features.unsqueeze(0)  # [1, n_i, 2048]
            
            # No mask needed for self-attention
            transformed = self.transformer(bag_features)  # [1, n_i, 2048]
            
            # Pool the transformed features (mean pooling)
            pooled = transformed.mean(dim=1)  # [1, 2048]
            
            outputs.append(pooled)
        
        # Combine all bags
        h_bags = torch.cat(outputs, dim=0)  # [num_bags, 2048]
        
        # Final classification
        logits = self.classifier(h_bags)  # [num_bags, num_classes]
        Y_hat = torch.argmax(logits, dim=1)
        
        return logits, attention_outputs, Y_hat
